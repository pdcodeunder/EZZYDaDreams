//
//  ECarWeiZhangZhiFuViewController.m
//  ElectricCarRent
//
//  Created by 彭懂 on 16/4/13.
...
@interface ECarWeiZhangZhiFuViewController () <UIAlertViewDelegate>

@property (nonatomic, strong) ECarMapManager * mapManager;

@end

@implementation ECarWeiZhangZhiFuViewController

- (ECarMapManager *)mapManager
{
    if (!_mapManager) {
        _mapManager = [[ECarMapManager alloc] init];
    }
    return _mapManager;
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(weizhangnotificationClicked) name:@"weizhangzhifuwancheng" object:nil];
}

- (void)weizhangnotificationClicked
{
//    ECarConfigs * config = [ECarConfigs shareInstance];
//    [self showHUD:@"请稍等..."];
//    [[self.mapManager weiZhangKouKuanFinishOrderWithID:config.orignOrderNo Type:@"1" andPayMethod:@"1"] subscribeCompleted:^{
//        [self hideHUD];
//    }];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"付款成功" message:@"您的违章已经交由EZZY代办处理，请耐心等待结果。" delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
    alert.tag = 344;
    [alert show];
...
//    btn.frame = CGRectMake(0, 100, kScreenW, 40);
//    [btn setTitle:@"支付宝" forState:UIControlStateNormal];
//    [btn addTarget:self action:@selector(zhifubaoSendPay) forControlEvents:UIControlEventTouchUpInside];
//    [self.view addSubview:btn];
//    
//    UIButton *btn1 = [UIButton buttonWithType:UIButtonTypeSystem];
//    btn1.frame = CGRectMake(0, 200, kScreenW, 40);
//    [btn1 setTitle:@"微信" forState:UIControlStateNormal];
//    [btn1 addTarget:self action:@selector(sendPaymeihoutai) forControlEvents:UIControlEventTouchUpInside];
//    [self.view addSubview:btn1];
    ...
    priceLabel.text = [NSString stringWithFormat:@"￥%@", [ECarConfigs shareInstance].currentPrice];
...
{
    [ECarConfigs shareInstance].zhifuwancheng = 30;
    ...
    if (signedString != nil) {
        ...
        [[AlipaySDK defaultService] payOrder:orderString fromScheme:appScheme callback:^(NSDictionary *resultDic) {
            ...
            if (resultStatus.integerValue == 9000 || resultStatus.integerValue == 8000) {
                ...
                NSString * strTitle = @"";
                NSString * strMsg = @"";
                if ([ECarConfigs shareInstance].zhifuwancheng == 30) {
                    strTitle = @"付款成功";
                    strMsg = @"您的违章已经交由EZZY代办处理，请耐心等待结果。";
                }
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:strTitle message:strMsg delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
                alert.tag = 344;
                [alert show];
                [ECarConfigs shareInstance].zhifuwancheng = 0;
            } else {
