...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central{
    ...
    NSDictionary *userInfo = [[NSDictionary alloc] initWithObjectsAndKeys:central, @"centralStatus", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"PDCBCentralManagerState" object:nil userInfo:userInfo];
...
//    NSLog(@">>>外设连接断开连接 %@: %@\n", [peripheral name], [error localizedDescription]);
    ...
    NSLog(@"sdaf [[babySpeaker callback] blockOnDisconnect]   %@", [[babySpeaker callback] blockOnDisconnect]);
    if ([[babySpeaker callback] blockOnDisconnect]) {
        [[babySpeaker callback] blockOnDisconnect](central,peripheral,error);
    }
    
    //判断是否全部链接都已经段开
    if ([self findConnectedPeripherals].count == 0) {
        //停止扫描callback
        if([[babySpeaker callback] blockOnCancelAllPeripheralsConnection]){
            [[babySpeaker callback] blockOnCancelAllPeripheralsConnection](centralManager);
        }
